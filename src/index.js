...
async function fetchTpexUniverse(){
  ...
  for(const u of urls){
    try{
      const r=await fetch(u,{headers:{"accept":"text/html,*/*","user-agent":"Mozilla/5.0 (compatible; tw-stock-api/1.11.3)"}});
    ...
    try{
      const r=await fetch(u,{
        headers:{
          ...
          "user-agent":"Mozilla/5.0 (compatible; tw-stock-api/1.11.3)"
        }
      });
      const text=await r.text();
  ...
  async fetch(request, env) {
    ...
    if (env.ASSETS) {
      ...
      if (path.endsWith(".html") || path === "/" || path === "/scanner.html" || path === "/detail.html") {
        ...
        headers.set("X-App-Version","1.11.3");
