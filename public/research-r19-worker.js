...
self.onmessage=async ({data:input})=>{
  try{
    ...
    const controls=C.controlled(train,test,(f,i)=>self.postMessage({type:'progress',text:`控制價格後的回歸 ${i+1}/8：${f}`,value:.65+.1*i/8})),pool=C.candidates(options.candidates,options.seed),results=[];
