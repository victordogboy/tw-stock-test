...
function coreChipFreshness(arr,target,prior){
  const d=coreLatestDate(arr);
  if(!d)return 0;
  if(target&&d>=target)return 10;
  if(prior&&d>=prior)return 8;
  return 4;
}
function decisionConfidence(rows,margin=[],inst=[],daytrade=[],meta={}){
  const count=rows?.length||0;
  let s=count>=180?15:count>=120?12:count>=65?8:0;
  const lastRow=rows?.at?.(-1)||{};
  const prevRow=rows?.at?.(-2)||lastRow;
  const target=String(meta.chipBaseDate||lastRow.date||'');
  const prior=String(prevRow.date||'');
  s+=Number.isFinite(Number(lastRow.close))&&Number(lastRow.close)>0?20:0;
  s+=coreChipFreshness(margin,target,prior);
  s+=coreChipFreshness(inst,target,prior);
  s+=coreChipFreshness(daytrade,target,prior);

  const liveLike=meta.mode==='live'||meta.mode==='scenario';
  s+=liveLike?(meta.quoteValid===false?0:15):15;
  const vc=Math.max(0,Math.min(1,Number(meta.volumeConfidence??(liveLike?.6:1))));
  s+=Math.round(10*vc);

  if(Number.isFinite(Number(meta.finmindCompleteness))){
    s+=Math.round(Math.max(0,Math.min(100,Number(meta.finmindCompleteness)))*.10);
  }else{
    const sets=[margin,inst,daytrade].filter(x=>Array.isArray(x)&&x.length).length;
    s+=sets===3?10:sets===2?7:sets===1?4:1;
  }
  return Math.max(0,Math.min(100,Math.round(s)));
}
function buildScorePacket(rows,margin=[],inst=[],daytrade=[],meta={}){
  ...
  const actionRaw=Math.round(entry*.40+setup*.30+opportunity*.30);
  const gateReasons=[];
  ...
  return {
    R,E,setup,opportunity,entry,hold,
    action,actionRaw,
    confidence:decisionConfidence(rows,margin,inst,daytrade,meta),
    gateOK:gateReasons.length===0,gateReasons,
    risk,hardBroken,
